...
module DispatchPolicy
  ...
  def self.config
    @config ||= Config.new(
      ...
      # Closed-loop self-tuning. When :apply, the TickLoop runs
      # Stats.bottleneck for each round-robin policy AT THE END of
      # each loop and persists the recommended_config knobs
      # (batch_size, round_robin_quantum) to the DB-backed table
      # with source: "auto". The next TickLoop spawn picks them up
      # at boot via reload_policy_configs!. Set to :recommend to
      # log without writing, or false to disable.
      auto_tune:             false
    )
  end
