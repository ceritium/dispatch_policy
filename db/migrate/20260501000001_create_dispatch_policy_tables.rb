...
class CreateDispatchPolicyTables < ActiveRecord::Migration[7.1]
  def change
    create_table :dispatch_policy_staged_jobs do |t|
      ...
      t.datetime :enqueued_at,  null: false, default: -> { "(now() AT TIME ZONE 'UTC')" }
    ...
    create_table :dispatch_policy_partitions do |t|
      ...
      t.datetime :scheduled_eligible_at
    ...
    create_table :dispatch_policy_inflight_jobs do |t|
      ...
      t.datetime :admitted_at,    null: false, default: -> { "(now() AT TIME ZONE 'UTC')" }
      t.datetime :heartbeat_at,   null: false, default: -> { "(now() AT TIME ZONE 'UTC')" }
    end
    ...
    create_table :dispatch_policy_tick_samples do |t|
      t.string   :policy_name,        null: false
      t.datetime :sampled_at,         null: false, default: -> { "(now() AT TIME ZONE 'UTC')" }
